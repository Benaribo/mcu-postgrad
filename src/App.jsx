...
export default function App() {
  ...
  const ReportView = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100 no-print">
        ...
        <button onClick={() => window.print()} className="flex items-center px-4 py-2 bg-gray-800 text-white rounded-lg"><Printer size={18} className="mr-2" /> Print PDF</button>
      </div>
  ...
  const StaffView = () => (
    <div className="space-y-4">
      <div className="flex justify-between bg-white p-4 rounded-xl border border-gray-100">
        ...
        <button onClick={() => { setSelectedStaff(null); setStaffForm({ name: '', email: '', department: 'Computer Science', password: 'staff' }); setIsStaffModalOpen(true); }} className="px-4 py-2 bg-indigo-600 text-white rounded-lg flex items-center"><Plus size={16} className="mr-2" /> Add Staff</button>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {staff.map(s => (
          <div key={s.id} className="bg-white p-4 rounded-xl border border-gray-100 flex justify-between items-center">
            <div><h3 className="font-bold">{s.name}</h3><p className="text-sm text-gray-500">{s.email}</p></div>
            <div className="flex gap-2">
              <button onClick={() => { setSelectedStaff(s); setStaffForm(s); setIsStaffModalOpen(true); }} className="p-2 text-gray-600 bg-gray-50 rounded-lg"><Edit size={16}/></button>
              <button onClick={() => handleDeleteStaff(s.id)} className="p-2 text-red-600 bg-red-50 rounded-lg"><Trash2 size={16}/></button>
            </div>
          </div>
        ))}
      </div>
  ...
  const FiltersBar = () => (
    <div className="flex gap-2">
      <select value={selectedSupervisor} onChange={e => setSelectedSupervisor(e.target.value)} className="border rounded-lg px-2 py-1 text-sm bg-white"><option>All Supervisors</option>{uniqueSupervisors.filter(x => x !== 'All Supervisors').map(x => <option key={x}>{x}</option>)}</select>
      <select value={selectedProgram} onChange={e => setSelectedProgram(e.target.value)} className="border rounded-lg px-2 py-1 text-sm bg-white"><option>All Programs</option>{Object.keys(PROGRAM_STRUCTURE).map(p => <option key={p}>{p}</option>)}</select>
    </div>
  );
